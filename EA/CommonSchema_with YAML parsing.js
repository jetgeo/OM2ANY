		...
		} 	else if (pDef)
		{
			...
			else if (/^\s{6}\S/.test(rArray[0]))
			{
				...
				}	else if (strP == "items")	
				{
					...
					//split on "/", find last element, UpperCase first letter, add "Type"
					var p = r.lastIndexOf("/");
					var strItem = r.slice(p+1,-3);
					attr.Type = strItem[0].toUpperCase() + strItem.slice(1)  + "Type";
				}	else if (strP == "minItems")
				{
					//For arrays: minItems
					attr.LowerBound = rArray[1].trim();
					attr.UpperBound = "*";
				}	else if (strP == "maxItems")
				{
					//For arrays: maxItems
					attr.UpperBound = rArray[1].trim();
				}   else if (strP == "uniqueItems" || rArray[1].trim == "true")
				{
					attr.AllowDuplicates = false;
					attr.Update();
				}
